...
class ModelService:
    def __init__(self, model_path: str):
        ...
        logger.info(f"Initializing model with {len(self.class_names)} classes")
    ...
    def load_model(self, model_path: str):
        try:
            ...
            # Add error checking for model architecture
            ...
            head_weight_shape = state_dict['vit.heads.head.weight'].shape
            logger.info(f"Checkpoint head shape: {head_weight_shape}")
            logger.info(f"Current model head shape: {self.model.vit.heads.head.weight.shape}")
            
            if head_weight_shape[0] != len(self.class_names):
                raise ValueError(
                    f"Model checkpoint has {head_weight_shape[0]} classes "
                    f"but expected {len(self.class_names)} classes. "
                )
...
async def startup_event():
    ...
    try:
        ...
        current_file = Path(__file__).resolve()  # Get the current file's absolute path
        logger.info(f"Current file path: {current_file}")
        
        backend_dir = current_file.parent.parent  # Go up one level to backend directory
        logger.info(f"Backend directory: {backend_dir}")
        
        model_path = backend_dir / "model_checkpoints" / "short_best_model.pth"
        ...
        if not model_path.exists():
            ...
            if model_dir.exists():
                ...
                logger.error(f"Available model files: {available_models}")
...
# Move the AudioRequest class before the app.post decorator
